...
def sama_features(df):
    ...
    # ⚠️ تم إيقاف حذف الأعمدة (للاختبار ومعرفة تأثيرها الكامل)
...
def filter_features(df, variance_threshold=0.99, correlation_threshold=0.85):
    ...
    if cols_to_drop_correlation:
        ...
        print(f"✂️ Removed {len(cols_to_drop_correlation)} Highly Correlated Features.")
...
def engineer_interaction_features(df):
    df = df.copy()
    print("\n" + "=" * 50)
    print("🧬 Adding Mathematically Correct Interaction Features...")
    ...
    # ⚠️ التعديل الرياضي العبقري (لأن الأعمدة دي Logarithmic)
    if 'SteamSpyPlayersEstimate' in df.columns and 'SteamSpyOwners' in df.columns:
        # log(Players / Owners) = log(Players) - log(Owners)
        df['Log_Active_Player_Ratio'] = df['SteamSpyPlayersEstimate'] - df['SteamSpyOwners']

    if 'SteamSpyOwners' in df.columns and 'PriceFinal' in df.columns:
        # log(Owners * Price) = log(Owners) + log(Price)
        df['Log_Estimated_Revenue'] = df['SteamSpyOwners'] + np.log1p(df['PriceFinal'])

    print(f"✅ Added Interaction Features. New Shape: {df.shape}")
    return df


def train_stacked_models(df, target_col='RecommendationCount'):
    print("\n" + "=" * 50)
    print("🎯 Training Level 2 Stacking Ensemble (XGBoost + LightGBM -> Ridge)")
    print("=" * 50)
    ...
    # 1. إعداد الـ Base Models (المستوى الأول) مع تزويد الأشجار لـ 800
    estimators = [
        ...
        ('lgb', lgb.LGBMRegressor(
            n_estimators=800, learning_rate=0.01, max_depth=8, num_leaves=63,
            subsample=0.8, colsample_bytree=0.8,
            random_state=42, n_jobs=-1, verbose=-1
        ))
    ]

    # 2. إعداد الـ Meta-Model (المستوى الثاني)
    # الموديل ده هيتعلم من أخطاء XGBoost و LightGBM ويدمجهم بذكاء
    stacked_model = StackingRegressor(
        estimators=estimators,
        final_estimator=RidgeCV(),  # Ridge بيبدع في دمج الموديلات ومنع الـ Overfitting
        cv=5,  # 5-Fold Cross Validation داخلي عشان الـ Meta-Model يتعلم صح
        n_jobs=-1,
        passthrough=False  # الموديل النهائي هيعتمد بس على توقعات الموديلين مش الداتا الأصلية
    )

    print("⏳ Training Stacked Model (This takes a moment, making 5 internal CV folds)...")
    stacked_model.fit(X_train, y_train)

    # 3. استخراج التوقعات
    stacked_preds_log = stacked_model.predict(X_test)

    # 4. حساب الأخطاء
    y_test_actual = np.expm1(y_test)

    stacked_preds_log_clipped = np.clip(stacked_preds_log, a_min=None, a_max=20)
    stacked_preds_actual = np.expm1(stacked_preds_log_clipped)

    r2 = r2_score(y_test, stacked_preds_log)
    rmse = np.sqrt(mean_squared_error(y_test_actual, stacked_preds_actual))
    mae = mean_absolute_error(y_test_actual, stacked_preds_actual)
    ...
    # طباعة الأوزان اللي الـ Meta-Model اختارها
    print(f"🧠 Meta-Model Weights (XGBoost vs LightGBM): {stacked_model.final_estimator_.coef_}")
...
df_with_interactions = engineer_interaction_features(df_golden) # الدالة المعدلة
final_stacked_model = train_stacked_models(df_with_interactions) # الدالة الجديدة